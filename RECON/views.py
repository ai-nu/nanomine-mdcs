	...
	if request.user.is_authenticated():
	    ...
	    if request.method == 'POST':
			...
			if form.is_valid():
				...
				os.system('matlab -nodesktop -nodisplay -nosplash -r "cd /home/NANOMINE/Production/mdcs/RECON/mfiles;MAIN(\'"'+user_name+'"\',"'+str(input_type)+'"\',"'+str(num_recon)+'",\'"'+str(file)+'"\');exit"')
						
				mail_to_user = 'sendmail ' + user_email_id + ' < /home/NANOMINE/Production/mdcs/RECON/email.html'
				os.system(mail_to_user)
				os.system('sendmail back2akshay@gmail.com < /home/NANOMINE/Production/mdcs/RECON/email.html')
				form = DocumentForm()
	    ...
	    # Render list page with the documents and the form
	        return render_to_response('RECON_submit_image.html',{'documents': documents, 'form': form},context_instance=RequestContext(request))
	else:
...
def viewResultRecon(request):
	if request.user.is_authenticated():
		form = DocumentForm()
		documents = Document.objects.all()
		return render_to_response('RECON_view_Result.html',{'documents': documents, 'form': form},context_instance=RequestContext(request))
	else:
		return redirect('/login')
