function [NP_center,NP_center_sub] = MC_generate_np_dist(VF,NP_radius,mean_num_NP,dev,lx,ly,lz,type,plot)
tic
...
min_d   = 2*NP_radius;                   % non-penerating radius of each NP
max_num_NP = round(mean_num_NP + 4*dev); % maximum number NP per cluste, ignore the portion away from 4 sigma
    ...
    for i=sum(num_cluster(1:j-1))+1:1:sum(num_cluster(1:j)) % every cluster within each type, total number num_cluster(j),ith cluster
        ...
        for k=2:1:j % j NP in each cluser
            ...
            while flg2 == 1
                ...
                conflict = 0;
                for ii = 1:1:k-1
                    if pdist([trial_coord;center(ii,:)]) < min_d
                        conflict = 1;
                    end
                end
