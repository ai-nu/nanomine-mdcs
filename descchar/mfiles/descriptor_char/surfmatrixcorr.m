function corrf = surfmatrixcorr(image)
% -------------------------------------------------------------------------
% Written by Hongyi Xu, Northwestern U
% 5/2/2013
% Surface-matrix correlation function.But it is normalized.
% 
% INPUT:
...
% ------------- Get the "matrix image", '1' mark the matrix----------------
[x,y]=size(image);
...
A = img - mean(mean(img));
B = imgm - mean(mean(imgm));

FA = fft2(A);
FB = fft2(B);

c = fftshift(ifft2(FA.*conj(FB))); % 2-points correlation. fftshift is to move the peak in the center of the image
