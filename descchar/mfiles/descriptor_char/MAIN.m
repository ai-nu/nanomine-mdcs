...
disp(strcat('There are_',num2str(num_img),'_images to be processed'));
...
% _v means the variance of the descriptor values
    ...
    current_file = strcat('./imgs/',img_list(ii).name);
    load(current_file,'img_out');  
    side_length = min(size(img_out)); 
%     for jj = 1:1:5 % should be 1:1:5
        
        name = img_list(ii).name;
...
%             disp('Finish filtering process 1');
            %tic; image_f = medfilt2(img, [150 150]); toc;
...
%             [~, Bimg] = Transform(img, VF(ii)); 
            Bimg = img_out(1:(side_length-10),1:(side_length-10));
            % choose the cut size
            cs = 1020;
            Bimg1 = Bimg;
%             Bimg1 = Bimg( :, 1:cs );
...
%             
            ...
            % Jan 28, 2015. Calculate area weighted equivalent radius
            awrc = rc1(:).*area1(:)./(sum(area1(:))/n1);
            
            % Jan 28, 2015. He Zhao. Calculate aspect ratio and area
            % weighted AR
            ar1 = zeros(length(els1),1);
            for jj = 1:1:length(els1)
                ar1(jj) = 1/els1(jj);
            end
            awar = ar1(:).*area1(:)./(sum(area1(:))/n1);
...
%             P_ncd = [ P_ncd ; mean(ncd2) var(ncd2) skewness(ncd2) kurtosis(ncd2) ];
            ...
            P_awrc = [P_awrc; mean(awrc) var(awrc) skewness(awrc) kurtosis(awrc)];
            
            P_awar = [P_awar; mean(awar) var(awar) skewness(awar) kurtosis(awar)];
%         end
...
save(save_path);
disp([img_list(ii).name '_is done.']);
